...
The manifest-to-kernel compiler, Landlock allowlist planner, fixed-size rule-map ABI, optional BPF-LSM `file_open` source, userspace loaders, Go OverlayFS manager, protected-run coordinator, policy-aware helper, atomic run store, cgroup-v2 scope, capability probe, event evidence digest with kernel-side dropped-event accounting, sensor start gate, merged-view diff, hash-chain evidence verifier, and `run/status/events/verify/diff/rollback/recover` CLI paths are implemented. The disposable VM reports Landlock and cgroup-v2 active and BPF-LSM absent. The Landlock child-process test, isolated OverlayFS rollback test, full FUSE protected-run smoke, descendant telemetry smoke, short-run sensor-gate smoke, cgroup drain/fail-closed smoke, parent-`SIGKILL` and open-descriptor stale-run recovery smokes, and warm/cold B0/B2/B4 benchmark controls passed with generated fixtures. The MVP boundary and the first Phase 2 P0 slice are documented as complete; the remaining work is the six-day hardening sprint described in [`docs/PHASE2_PLAN.md`](PHASE2_PLAN.md): broader crash edge coverage, bounded event rotation, network/credential policy planes, conflict-safe commit/export, and release rehearsal.
...
The first full synthetic FUSE run and rollback passed in the disposable VM: read denial, deletion isolation, eBPF telemetry, successful lifecycle recording, unmount, upper/work discard, and lower-layer preservation were all observed. Phase 2 restores record and event-log ownership to the invoking user after `sudo`; privileged FUSE unmount and rollback still require `sudo`.
...
The B4 protected-run measurement used five fio repetitions inside one FUSE-backed Rewind run. Mean throughput was 36,726 KiB/s / 9,181.7 IOPS read and 15,730 KiB/s / 3,932.6 IOPS write, approximately 11.1% below B0 and 0.4% above B2. The complete run took 64.34 seconds wall-clock. A direct-fio PID telemetry validation recorded 16,620 events (16,403 writes, 216 opens, one unlink) in a 2,467,528-byte log before rollback. Descendant tracking was then verified with a shell-launched `/bin/dd`: 46 events across two PIDs (15 writes, 30 opens, one execve), followed by a successful rollback. Phase 2 now creates one cgroup scope per run and refuses to claim clean completion if child processes remain. The short drop-counter smoke recorded 39 events (7,334 bytes) with `dropped=0` and rolled back successfully.
