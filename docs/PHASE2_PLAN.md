...
3. The event stream is JSONL and can grow much faster than the run record; kernel-side reserve failures are now counted and make evidence incomplete. A bounded `REWIND_EVENT_MAX_BYTES` cap now marks userspace truncation explicitly, and the read-only evidence verifier checks the digest and chain; multi-file rotation and backpressure remain future work.
