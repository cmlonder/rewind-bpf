...
| Windows native backend | **Prepared / manual gate** | Cross-build, fail-closed capability report, and read-only PowerShell/fsutil prerequisite plan; WSL2 remains compatibility mode | Native process/filesystem policy + disposable workspace implementation and Windows VM tests |
| Agent integrations | **Partial / lifecycle contract** | `--agent-adapter` validates and persists identity; `agent.Prepare` preserves the command and injects `REWIND_AGENT_ADAPTER`; `rewind agent list` exposes the registry | SDK-specific launch semantics, callbacks, and provider tests |
...
| Detachable/ghost sessions | **Partial / local multi-process lease** | Expiring authenticated acquire/heartbeat/takeover/release owner leases, reconnectable event follow, redacted session audit, and atomic cross-process lock file | Runtime-enforced multi-operator coordination and distributed session storage |
| Content-aware PII protection | **Partial / pre-read enforcement** | Deterministic bounded scanner detects common PII/token patterns; audit findings contain hashes only; `read.pii.mode: enforce` turns findings into exact Landlock denies before agent start; optional redacted output | Configurable classifiers, streaming/new-file coverage, and leakage benchmarks |
| Multi-agent/checkpoint graph | **Partial / local graph foundation** | Durable dependency graph with parent validation, deterministic state transitions, pending-child merge guard, and `rewind checkpoint graph` CLI | Child-run orchestration, dependency-aware merge, and multiple rewind points wired to live transactions |
