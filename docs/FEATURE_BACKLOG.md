...
| Detachable/ghost sessions | **Partial / local + remote protocol** | Expiring authenticated acquire/heartbeat/takeover/release owner leases, reconnectable event follow, redacted session audit, atomic cross-process lock file, and bearer-authenticated remote lease client with bounded retries | SQLite-backed index and distributed deployment/consensus |
| Content-aware PII protection | **Partial / bounded lifecycle coverage** | Deterministic bounded scanner detects common PII/token patterns; audit findings contain hashes only; `read.pii.mode: enforce` turns findings into exact Landlock denies before agent start; protected runs rescan newly-created files after exit | Configurable classifiers, streaming coverage, and leakage benchmarks |
