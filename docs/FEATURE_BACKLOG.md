...
| Network policy | **Partial** | Loopback HTTP/HTTPS proxy backend with allow/audit/deny decisions persisted in the run evidence chain for proxy-aware clients | Network namespace/cgroup egress enforcement and raw-socket coverage |
...
| Signed policy provenance | **Shipped / local** | Ed25519 keygen/sign/verify policy bundles | Remote registry, revocation, and organization trust distribution |
...
| Control Plane UI | **Shipped / fixture + authenticated bridge** | Responsive operational views, policy/config/workspace management fixtures, evidence/benchmark/audit views, loopback HTTP supervisor bridge, bearer-authenticated rollback/recover/commit intents, authenticated SSE evidence follow | Connected policy/workspace mutation, reconnect UX, local action-token UX |
...
| Linux release/bootstrap | **Shipped** | VM bootstrap, release Make targets, Linux-only benchmark guard, Go/UI CI, cross-build checks | Signed release artifacts and package repository |
