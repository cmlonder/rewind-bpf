...
export function Roadmap() {
  return `<section class="roadmap-section section-shell" id="roadmap" aria-labelledby="roadmap-title">
    ...
    <div class="roadmap-list">${siteData.roadmap.map((item, i) => `<article class="roadmap-item reveal reveal-delay-${i}"><div class="roadmap-phase">${item.phase}</div><div class="roadmap-copy"><div class="roadmap-title"><h3>${item.title}</h3><span class="roadmap-status ${i === 0 ? "is-done" : ""}">${item.status}</span></div>${item.tags?.length ? `<div class="roadmap-tags" aria-label="${item.phase} scope">${item.tags.map((tag) => `<span>${tag}</span>`).join("")}</div>` : ""}<p>${item.body}</p></div><div class="roadmap-index">0${i + 1}</div></article>`).join("")}</div>
    ...
    <div class="roadmap-foot reveal"><span>Next proof point</span><strong>Signed native helper gates for macOS EndpointSecurity and Windows minifilter/VHDX</strong><span class="roadmap-line"></span><a href="https://github.com/cmlonder/rewind-bpf/blob/main/docs/FEATURE_BACKLOG.md" target="_blank" rel="noreferrer">Read feature backlog ↗</a></div>
  </section>`;
}
