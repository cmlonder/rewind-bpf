...
The optional `rewind pii scan` command is an audit/redaction helper. In a protected policy, `read.pii.mode: audit` records hashed findings and `read.pii.mode: enforce` turns the pre-run findings into exact Landlock denies; neither mode returns raw matches or grants access. The scanner is bounded to 8 MiB text files, and the protected run performs a second bounded scan after exit so newly-created files can be attached to the record without exposing raw values. Choose an adapter identity with `--agent-adapter generic|codex|openhands|claude-code`; the identity is persisted in the run plan and exposed to the child as `REWIND_AGENT_ADAPTER`, while the operator-owned command remains unchanged.
