...
Stage 6 is in progress. Stage 5 read enforcement and Stage 3 OverlayFS rollback are validated in the disposable VM: safe synthetic fixtures, SHA-256 manifests, run IDs, glob policy parsing, a protected-run state machine, a shared eBPF event contract, a userspace ring-buffer decoder/reader, a scoped telemetry loader, a manifest-to-kernel read-rule compiler, a Landlock allowlist planner, a process-level Landlock denial test, a Go OverlayFS mount/rollback test, a protected-run plan composer, a fail-closed coordinator, and a policy-aware hidden helper process are available. The VM reports Landlock active, so Landlock is the primary read-enforcement path; eBPF remains the low-overhead telemetry path. The next implementation step is wiring these pieces into the user-facing `rewind run` flow.
