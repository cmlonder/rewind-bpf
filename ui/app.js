...
function bindInteractions() {
  ...
  if (search) search.addEventListener("input", () => filterRuns(search.value, "all"));
  document.querySelectorAll("[data-run-filter]").forEach((element) => element.addEventListener("click", () => {
    ...
    filterRuns(search?.value || "", element.dataset.runFilter);
  }));
}
...
function copyPolicy() {
  ...
  setToast("Policy copied to clipboard preview.", "success");
}
...
function openPolicyEditor() {
  openModal("New policy package", `<form id="policy-form" class="modal-form"><label>Package name<input name="name" value="review-safe" required /></label><label>Version<input name="version" value="0.1.0" required /></label><label>Description<textarea name="description">Review-first profile for a new workspace.</textarea></label><div class="form-note">New packages start in review mode and apply only to future runs.</div></form>`, { confirm: "Create package", onConfirm: () => {
...
function openWorkspaceEditor(name = "") {
  openModal(name ? `Edit ${name}` : "Add workspace", `<form id="workspace-form" class="modal-form"><label>Workspace name<input name="name" value="${name}" required /></label><label>Workspace path<input name="path" value="/workspaces/${name || "new-project"}" required /></label><label>Policy package<select name="policy"><option>strict-agent@1.3.0</option><option>developer-safe@0.8.2</option><option>hackathon-demo@0.4.0</option></select></label><div class="form-note">Assignment is stored as a revision and used by new runs only.</div></form>`, { confirm: name ? "Save assignment" : "Add workspace", onConfirm: () => {
...
function openModal(title, body, { confirm, tone = "orange", onConfirm }) {
  ...
  layer._onKeyDown = (event) => { if (event.key === "Escape") closeModal(); };
